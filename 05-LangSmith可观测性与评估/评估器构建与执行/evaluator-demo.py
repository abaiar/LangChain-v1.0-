"""
第六章 6.3.3 评估器的构建与执行 — LangSmith 评估器示例

【章节学习重点】
- LangSmith 评估器的构建方式：自定义评估逻辑
- LLM-as-a-judge 模式：使用 LLM 评估另一个 LLM 的输出质量
- 评估器的执行与结果分析

【代码功能】
演示如何构建和执行 LangSmith 评估器，
使用 LLM-as-a-judge 模式对 AI 应用的输出进行自动化评估。

【实现思路】
1. 加载环境变量
2. 定义评估器逻辑（LLM-as-a-judge 或规则评估）
3. 执行评估并收集结果

【关键参数说明】
- LLM-as-a-judge: 使用 LLM 作为评判者评估输出质量
- evaluate(): 执行评估函数
- 评估指标: 准确性、相关性、完整性等
