"""
第三章 3.4.3 集成公司规章制度知识库 — RAG 知识库核心模块

【章节学习重点】
- RAG 知识库的完整生命周期管理：初始化、查询、工具封装
- Milvus 向量数据库的连接管理与缓存策略
- 将 RAG 检索能力封装为 LangChain Tool 供智能体调用
...
【实现思路】
1. 配置常量：PDF路径、Milvus连接参数、Embedding模型参数
2. Milvus 连接管理：幂等连接函数，支持重连
3. 向量存储管理：使用 lru_cache 缓存 Embedding 和 Milvus 实例
4. 知识库初始化：加载PDF → 切分 → 向量化 → 写入Milvus，支持删除重建
5. 知识库查询：连接检查 → 相似度检索 → 格式化返回
6. Tool 封装：使用 @tool 装饰器将检索函数注册为 LangChain 工具

【关键参数说明】
- PROJECT_ROOT: 项目根目录，用于定位 PDF 文件
- MILVUS_URI: Milvus 服务地址
- COLLECTION_NAME: Milvus 集合名称
- EMBEDDING_MODEL: DashScope Embedding 模型名称
- delete_existing: 是否删除现有集合后重建
- @tool: LangChain 工具装饰器，自动生成工具 Schema

【应用场景】
- 企业内部规章制度、员工守则等私有知识的 RAG 检索
- 智能体集成私有知识库的标准模式
- 知识库的按需初始化和更新管理
"""
import os
