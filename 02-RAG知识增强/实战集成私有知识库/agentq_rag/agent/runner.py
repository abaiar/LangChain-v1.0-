...
"""
第三章 3.4.5 修改智能体调度器 — Agent Runner 模块（集成RAG）

【章节学习重点】
- 在原有 Agent 基础上集成 RAG 知识库工具
- 消息过滤策略：只保留 HumanMessage 和最终 AIMessage，过滤 ToolMessage
- LangGraph Runtime 兼容性处理

【代码功能】
对 LangChain create_agent 进行最小封装，结合 MCP 工具、RAG 知识库工具与中间件。
提供同步调用入口，方便在 Streamlit 等传统 Web 环境中复用。
暴露 build_runner() 供应用层按需构建预配置的 AgentRunner。

【实现思路】
1. 兼容处理：对 langgraph.runtime API 变动导致的导入失败进行修补
2. AgentRunner 类：封装 agent 和 memory，提供 invoke() 同步调用方法
3. invoke() 方法流程：拼接历史+当前输入 → 调用Agent → 过滤消息 → 回写记忆 → 返回文本
4. build_runner() 工厂方法：组装模型+MCP工具+RAG工具+中间件+记忆

【关键参数说明】
- recursion_limit: Agent 递归调用限制，设为50允许更多工具调用轮次
- filtered_messages: 过滤后的消息列表，只保留 HumanMessage 和无 tool_calls 的 AIMessage
- mcp_tools: MCP 工具列表（math + weather）
- kb_tool: RAG 知识库工具（company_policy_knowledge_base）

【应用场景】
- 在原有问答智能体基础上扩展私有知识库能力
- 智能体需要同时调用外部API工具和内部知识库的场景
- Streamlit 等 Web 应用的后端 Agent 调度
"""
