"""
第三章 3.4 实战集成私有知识库 — 中间件总装配模块

【章节学习重点】
- 三类中间件的完整装配：预置中间件、装饰器中间件、类中间件
- 中间件栈的组装顺序与执行优先级
- DeepSeek 兼容层：消息内容规范化处理

【代码功能】
中间件总装配，覆盖三类形态：
1) 预置（Built-in）中间件：直接复用官方提供的能力（SummarizationMiddleware）
...
【实现思路】
1. 消息规范化：_stringify_message_content 和 _normalize_request_messages 处理 DeepSeek 兼容
2. 预置中间件：SummarizationMiddleware 自动摘要长对话
3. 装饰器中间件：
   - rate_guard: 限频保护（1秒内不重复请求）
   - clarify_missing_params: 参数澄清引导（天气缺城市/数学缺表达式）
   - post_format_checker: 模型输出标注
   - post_agent_logger: Agent 完成打点
   - night_briefing: 夜间简洁模式
4. 类中间件：
   - TimingAndRetryMiddleware: 计时+重试
   - ConversationGuardMiddleware: 输入长度校验+历史过多提示+输出截断
5. install_default_middlewares(): 统一装配出口

【关键参数说明】
- max_tokens_before_summary: 触发摘要的 token 阈值
- messages_to_keep: 摘要后保留的最近消息数
- max_input_chars: 输入最大字符数限制
- max_output_chars: 输出最大字符数限制

【应用场景】
- 生产环境智能体的完整治理中间件栈
- 多种中间件形态的组合使用范例
- DeepSeek 等非 OpenAI 模型的兼容处理
"""
