"""
第四章 4.2.7 快速上手创建第一个LangGraph程序 — 完整智能问答系统

【章节学习重点】
- StateGraph 的完整构建流程：状态定义、节点注册、边定义、条件边、编译执行
- Functional API（@task/@entrypoint）的使用：轻量级任务定义和入口点
- Send 并行分发机制：将任务并行发送到多个节点
- Runtime 上下文：通过 context_schema 传递运行时配置
- Annotated 类型注解：使用 operator.add 实现状态字段的累加语义

【代码功能】
构建一个完整的 LangGraph 智能问答系统，包含问题分析、章节规划、并行展开、
总结回答等完整流程。支持 DeepSeek 分析 + 通义千问生成的双模型协作。

【实现思路】
1. 定义 LLM 调用封装（deepseek_chat/qwen_chat）和工具函数
2. 定义 QAState 状态类，使用 Annotated 实现列表累加和计数累加
3. 使用 @task/@entrypoint 定义 Functional API 规划工作流
4. 定义图节点：analyze_question → plan_with_functional_api → dispatch → expand_section → collect_sections → summarize
5. 使用 add_conditional_edges 实现规划后的路由决策
6. 使用 Send 实现章节并行展开
7. 编译图并执行

【关键参数说明】
- StateGraph(QAState, context_schema=AssistantContext): 状态图，QAState为状态类型，context_schema为运行时上下文
- Annotated[List[str], operator.add]: 列表类型，多节点返回值自动合并（而非覆盖）
- Annotated[int, operator.add]: 整数类型，多节点返回值自动累加
- Send("expand_section", {...}): 并行发送，将数据发送到指定节点
- InMemorySaver(): 内存检查点，用于 Functional API 的状态持久化
- Runtime[AssistantContext]: 运行时上下文，通过 runtime.context 访问配置
...
def summarize_answer(state: QAState, runtime: Runtime[AssistantContext]) -> Dict[str, Any]:
    ...
    analysis_text = "\n".join(state.get("analysis", []))
    plan_text = "; ".join(state.get("plan", [])) or "（无显式章节）"
