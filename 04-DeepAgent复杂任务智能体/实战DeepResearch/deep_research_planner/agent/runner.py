"""
第五章 5.4.6 集成Deep Agent调度器与人工协作机制 — Agent Runner 模块

【章节学习重点】
- Deep Agent 的构建与配置：create_deep_agent 的完整参数使用
- 子智能体、后端、中间件、人工协作的集成方式
- Agent 调用的封装：invoke 和 resume 的统一入口

【代码功能】
Agent 构建与调用入口。负责创建配置完备的 Deep Agent 实例，
并提供调用和恢复执行的封装函数。

【关键参数说明】
- create_deep_agent: 创建 Deep Agent，配置模型/工具/子智能体/后端/中间件
- invoke/resume: 执行和恢复的封装方法

【应用场景】
- Deep Research 智能体的后端调度入口
- Streamlit 前端与 Deep Agent 的桥接层
"""
