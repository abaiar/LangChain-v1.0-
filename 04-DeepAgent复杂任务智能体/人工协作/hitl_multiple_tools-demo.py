"""
第五章 5.3.3 人工协作 — 多工具调用的 HITL 示例

【章节学习重点】
- 多工具同时调用的 HITL 行为：多个工具同时触发中断
- 一次性中断 vs 逐个中断：多个需要审批的工具同时调用时的处理方式

【代码功能】
一个最小可运行示例：展示 Multiple Tool Calls 的 HITL 行为。
场景：用户输入 "请同时备份文件和清理临时目录"，
智能体会规划调用两个工具：backup_data 和 cleanup_temp。
这两个工具都配置成需要人工审批，因此会一次性触发中断。

【关键参数说明】
- interrupt_on: 配置多个需要审批的工具
- 多工具中断: 一次 invoke 可能触发多个工具的中断
